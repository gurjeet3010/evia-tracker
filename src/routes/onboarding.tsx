...
import { addPeriod } from "@/lib/cycleApi";
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-muted">
              <div className="h-full rounded-full bg-gradient-pink transition-all duration-500" style={{ width: `${progress}%` }} />
            </div>
          </div>
        </div>
          ...
          <div className="rounded-3xl border border-border/60 bg-card/80 p-6 shadow-soft backdrop-blur">
            {step === 1 && (
              <StepWrapper icon={<User className="h-6 w-6" />} title="What should we call you?" subtitle="Your name helps make Evia feel personal.">
                <input autoFocus type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your first name" maxLength={40}
            ...
            {step === 2 && (
              <StepWrapper icon={<Heart className="h-6 w-6" />} title="Who are you tracking for?" subtitle="You can track your own cycle or support someone you love.">
                <div className="grid gap-3">
                  <ChoiceCard selected={trackingFor === "self"} onClick={() => setTrackingFor("self")} emoji="🌸" title="Myself" description="Track your own cycle and wellness." />
                  <ChoiceCard selected={trackingFor === "partner"} onClick={() => setTrackingFor("partner")} emoji="💞" title="My partner" description="Stay in sync and support them through every phase." />
                </div>
              </StepWrapper>
            )}
            {step === 3 && (
              <StepWrapper icon={<CalendarIcon className="h-6 w-6" />} title="When did the last period start?" subtitle="A rough date is fine — you can update it anytime.">
                <input type="date" value={lastPeriod} max={new Date().toISOString().slice(0, 10)} onChange={(e) => setLastPeriod(e.target.value)}
                  className="w-full rounded-2xl border border-input bg-background px-4 py-3.5 text-base outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20" />
              </StepWrapper>
            )}
            {step === 4 && (
              <StepWrapper icon={<Flower2 className="h-6 w-6" />} title="Cycle & period length" subtitle="The average is 28 days, but every body is different.">
                <div className="space-y-5">
                  <SliderField label="Cycle length" value={cycleLength} unit="days" min={21} max={45} onChange={setCycleLength} hint="Days between the start of two periods" />
                  <SliderField label="Period length" value={periodLength} unit="days" min={2} max={10} onChange={setPeriodLength} hint="How long the period typically lasts" />
                </div>
              </StepWrapper>
            )}
            {error && (
              <div className="mt-4 rounded-xl bg-destructive/10 px-4 py-2.5 text-xs font-medium text-destructive">{error}</div>
            )}
          </div>

          <div className="mt-6 flex items-center gap-3">
            {step > 1 && (
              <button type="button" onClick={back} className="flex items-center justify-center gap-1 rounded-2xl border border-border bg-card px-5 py-3.5 text-sm font-semibold text-foreground transition-colors hover:bg-muted">
                <ChevronLeft className="h-4 w-4" /> Back
              </button>
            )}
            <button type="button" onClick={next} disabled={busy} className="flex flex-1 items-center justify-center gap-1.5 rounded-2xl bg-gradient-pink py-3.5 text-sm font-semibold text-primary-foreground shadow-soft transition-transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60">
              {step === TOTAL_STEPS ? (<><Check className="h-4 w-4" /> {busy ? "Saving…" : "Finish setup"}</>) : (<>Continue <ChevronRight className="h-4 w-4" /></>)}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      <div className="mb-5 flex flex-col items-center text-center">
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-pink text-primary-foreground shadow-glow">{icon}</div>
...
function ChoiceCard({ selected, onClick, emoji, title, description }: { selected: boolean; onClick: () => void; emoji: string; title: string; description: string }) {
  return (
    <button type="button" onClick={onClick} className={`flex items-center gap-4 rounded-2xl border-2 p-4 text-left transition-all ${selected ? "border-primary bg-primary/5 shadow-soft" : "border-border bg-card hover:border-primary/40"}`}>
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-muted text-2xl">{emoji}</div>
      <div className="flex-1">
...
function SliderField({ label, value, unit, min, max, onChange, hint }: { label: string; value: number; unit: string; min: number; max: number; onChange: (v: number) => void; hint: string }) {
  return (
    <div>
      ...
      <div className="mt-1 flex justify-between text-[10px] text-muted-foreground"><span>{min}</span><span>{max}</span></div>
    </div>
  );
}
